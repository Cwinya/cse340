...
 * RENAMED to match controller's expected function: buildItemDetailHTML
 * Added reviewSectionHTML parameter
 * *************************************** */
Util.buildItemDetailHTML = async function(item, reviewSectionHTML = "") {  
...
    detailView += '<div class="review-section-wrapper">';
    detailView += reviewSectionHTML;
    detailView += '</div>';
...
/* ****************************************
 * Build the HTML for the reviews section (NEW)
 * *************************************** */
Util.buildReviewDisplay = async function (reviews) {
    if (!reviews || reviews.length === 0) {
        return "<p class='text-lg mt-4'>Be the first to leave a review!</p>"
    }

    let reviewHTML = "<div class='review-list space-y-6 mt-6'>"
    
...
        // Build star rating (simple string of stars)
...
                <span class="font-bold text-lg text-primary">${review.account_firstname.charAt(0).toUpperCase()}${review.account_lastname.charAt(0).toUpperCase()}</span>
                <span class="ml-3 font-semibold text-gray-800">${review.account_firstname} ${review.account_lastname}</span>
            </div>
            <div class="text-sm text-yellow-500 mb-1">${stars}</div>
            <p class="text-gray-600 italic text-sm">Reviewed on ${date}</p>
            <p class="mt-2 text-gray-700 leading-relaxed">${review.review_text}</p>
        </div>
        `
    })
...
    // RENAMED EXPORT to fix the "is not a function" error
    buildItemDetailHTML: Util.buildItemDetailHTML, 
...
    buildReviewDisplay: Util.buildReviewDisplay
}