...
// Helper to determine production status for cookie setting
...
/* ****************************************
 * Process login request (CRITICALLY UPDATED)
 * *************************************** */
async function accountLogin(req, res) {
    ...
    // Check if password matches (Robust comparison and error handling)
    try {
        ...
        if (passwordMatch) {
            ...
            // CRITICAL CHECK: Ensure ACCESS_TOKEN_SECRET is set
            ...
            // FIX: Set secure and sameSite properties for the JWT cookie
            res.cookie("jwt", accessToken, { 
                httpOnly: true, 
                maxAge: 3600 * 1000,
                // Apply production fixes for Render deployment
                secure: isProduction, 
        ...
        // Log bcrypt/JWT error, but proceed to fail login gracefully
        console.error("Login attempt failed (Bcrypt or JWT error):", error);
    }
    
    // If login failed (password mismatch, bcrypt error, or any other issue)
    req.flash("error", "Please check your credentials and try again.")
    return res.status(400).render("account/login", {
        title: "Login",
        nav,
...
async function updateAccount(req, res) {
    ...
    if (updateResult) {
        ...
        // FIX: Re-generate the token and set the cookie with production settings
